...
\linebreak W związku z tym, trudno jednoznacznie stwierdzić, która z metod jest najbardziej efektywna.
...
Poniżej wyróżniono najczęściej stosowane metody \linebreak w detekcji tablic rejestracyjnych.
...
Dla poprawienia widoczności numerów pojazdów, stosuje się kolory o wysokim kontraście dla czcionki \linebreak i tła.
...
Do tego typu metod wykorzystywany jest m.\ in.\ algorytm \textit{Mean shift}~\cite{1520110} \linebreak i logika rozmyta~\cite{Wang2008FuzzybasedAF}.
...
\begin{align}
    ...
    \end{aligned}
\end{align}
gdzie $s(j,k)$ to skumulowana suma w wierszu, $f(j,k)$ to wartość obrazu w punkcie $(j,k)$, natomiast $s(j,-1)=0$ oraz $ii(-1,k)=0$~\cite{990517}.
Dla $j=k=1$ zachodzi równość \linebreak $s(1,1)=f(1,1)$.
...
\end{equation}
Na podstawie powyższego wzoru można zauważyć, że wystarczą operacje tylko na \linebreak 4 punktach obrazu całkowego~\cite{szybka_detekcja_klesk}.
...
Dzięki zastosowaniu boostingu, algorytm dokonuje selekcji znacznie mniejszej liczby cech, które zapewnią dobry opis rozpatrywanych obiektów \linebreak w danym zagadnieniu detekcji.
...
Słabe klasyfikatory stosowane w algorytmach boostingowych mogą być realizowane na wiele sposobów.
...
W przeciwieństwie do tradycyjnych metod z oknem przesuwnym, YOLO podczas procesu uczenia \linebreak i testowania otrzymuje na wejściu cały obraz.
...
W~\cite{686064} autorzy skanowali w poziomie obraz binarny, w celu znalezienia obszarów, \linebreak w których stosunek pikseli tła do pikseli znaków przekraczał predefiniowany próg.
...
Taki sposób jest łatwy w implementacji, natomiast jest ściśle powiązany z konkretnym formatem tablic \linebreak i nie nadaje się do użycia dla różnych rodzajów.
...
Warto również wspomnieć, że jedna \linebreak z najpopularniejszych bibliotek programistycznych Tesseract~\cite{tesseract} od wersji 4 wykorzystuje sieci LSTM\@.
...
\end{figure}
\FloatBarrier
Cyfrową reprezentacją znaku jest tablica, gdzie 1 to punkt wchodzący w skład znaku, \linebreak a 0 to tło.

...
\end{figure}
\FloatBarrier
Im większa będzie ilość okien w procedurze skanującej, tym większa będzie niezbędna liczba cech do wyliczenia.
...
Sposobem na szybsze obliczenie obrazu całkowego może być wykorzystanie wzorów rekurencyjnych:
%\begin{equation}
%    \label{eq:integral_image_induction}
%    s(j,k)=s(j, k-1)+f(j,k)
%\end{equation}

to jest osobny akapit dotyczący szybkiego wyliczania sum jasności na podstawie obrazu całkowego
\\
...
Ten sposób charakteryzuje się stałą złożonością obliczeniową $O(1)$.
